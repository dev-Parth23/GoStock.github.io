...
const Dropbox = () => {
  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-white">
      {/* Header Section */}
      <header className="flex justify-between items-center pr-6 py-0 pl-24 bg-zinc-800">
        <div className="flex items-center space-x-4">
          <img src="https://static.cdnlogo.com/logos/d/38/dropbox.png" alt="Dropbox Inc." className="w-32 h-32 rounded-full" />
          <div className="text-2xl font-bold text-blue-500">
            {stockData.name} ({stockData.symbol})
          </div>
        </div>
        <div className="text-lg">
          <span className="text-blue-500">${stockData.price}</span>
        </div>
      </header>
      ...
      <main className="p-8 space-y-8 items-center justify-center">
        ...
        <section className="bg-zinc-800 p-6 rounded-lg shadow-lg">
          ...
          <p className="text-gray-400">
            Dropbox Inc. is a cloud-based file storage and collaboration platform, offering users a simple and secure way to store, access, and share files across devices.
          </p>
        </section>
        ...
        <section className="text-center space-x-4">
          {['1d', '1w', '1m', '6m', '1y', '5y', 'all'].map((timeframeOption) => (
            <button
              key={timeframeOption}
              className={`py-1 px-4 rounded-full transition duration-300 ${timeframe === timeframeOption ? 'bg-blue-500' : 'bg-gray-700'}`}
