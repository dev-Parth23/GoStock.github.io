...
const Airbnb = () => {
  ...
  // Predict the price based on the last price in the selected timeframe
  const predictPrice = () => {
    const lastPrice = stockData.historicalPrices[timeframe][stockData.historicalPrices[timeframe].length - 1];
    const predictedPrice = lastPrice * (1 + Math.random() * 0.05); // Random fluctuation up to 5%
  ...
  // Handle the change of the selected timeframe
  const handleTimeframeChange = (newTimeframe) => {
    setTimeframe(newTimeframe);
  };

  // Update the predicted price whenever the timeframe changes
  useEffect(() => {
    predictPrice();
  }, [timeframe]);
  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-white">
      {/* Header Section */}
      <header className="flex justify-between items-center pl-28 p-6 bg-zinc-800">
      ...
      <main className="p-8 space-y-8 items-center justify-center">
        ...
        <section className="bg-zinc-800 p-6 rounded-lg shadow-lg">
          ...
          <p className="text-gray-400">
            Airbnb Inc. is an online marketplace for lodging, primarily homestays for vacation rentals, and tourism activities.
            Airbnb allows people to list, discover, and book accommodations around the world.
          </p>
        </section>
